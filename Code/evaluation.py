...
_REL2ID_PATH = os.path.join(_SCRIPT_DIR, 'dataset', 'meta', 'rel2id.json')
try:
    rel2id = json.load(open(_REL2ID_PATH, 'r'))
except FileNotFoundError:
    # Kept import-safe for --help/setup checks; real evaluation still requires rel2id.
    rel2id = {}
...
def official_evaluate(tmp, path, train_file = "train_annotated.json", dev_file = "dev.json"):
    ...
    if os.path.exists(train_distant_path):
        ...
        print(f"[WARN] {train_distant_path} not found; IgnF1 will ignore only annotated-train facts.")
