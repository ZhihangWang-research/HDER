...
def collate_fn(batch):
    ...
    entity_type = [f.get("entity_type") for f in batch] if all("entity_type" in f for f in batch) else None
    ...
    if len(sent_labels) != 0 and None not in sent_labels:
        sent_labels_tensor = []
    ...
    output = (input_ids, input_mask, labels, entity_pos, hts, sent_pos, sent_labels_tensor, attns, entity_type)
